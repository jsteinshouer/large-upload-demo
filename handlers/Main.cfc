component extends="coldbox.system.EventHandler" {
	...
	function upload( event, rc, prc ){
		var filePath = "#getDirectoryFromPath(getBaseTemplatePath())#\.storage\#rc.filename#";
		fileMove(rc.file, filePath);
		return filePath;
	}

	function uploadChunk( event, rc, prc ){
		var uploadDir = "#getDirectoryFromPath(getBaseTemplatePath())#\.storage\#rc.fileID#"
		if ( !directoryExists(uploadDir) ) {
			directoryCreate(uploadDir);
		}

		fileAppend("#uploadDir#\#rc.fileName#", fileReadBinary(rc.chunk) );
